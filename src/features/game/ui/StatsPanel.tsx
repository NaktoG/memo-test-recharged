...
export const StatsPanel = ({ stats, activeLevel, onResetStats }: StatsPanelProps) => {
  ...
  return (
    <aside className="rounded-[2rem] border border-white/10 bg-slate-950/45 p-5 shadow-glow backdrop-blur-xl lg:sticky lg:top-6 lg:self-start">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.28em] text-fuchsia-200">Stats</p>
          <h2 className="mt-2 text-2xl font-black text-white">Tu progreso</h2>
        </div>
      ...
      <dl className="mt-6 grid grid-cols-2 gap-3">
        <Stat label="Jugadas" value={stats.gamesPlayed} />
        <Stat label="Win rate" value={`${winRate}%`} />
        <Stat label="Ganadas" value={stats.gamesWon} />
        <Stat label="Perdidas" value={stats.gamesLost} />
        <Stat label="Racha" value={stats.currentStreak} />
        <Stat label="Mejor racha" value={stats.bestStreak} />
      </dl>

      <div className="mt-5 rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-4">
        <p className="text-sm font-bold text-cyan-100">Mejor marca activa</p>
        <dl className="mt-4 grid grid-cols-2 gap-3">
          <Stat label="Tiempo" value={formatNullableTime(best.bestTimeSeconds)} compact />
          <Stat label="Score" value={best.bestScore} compact />
        </dl>
      </div>
    </aside>
  );
